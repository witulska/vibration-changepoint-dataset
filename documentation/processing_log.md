...
### Measurement 01 (`pomiar_nr1.csv`)

- Source encoding / format: `utf-8-sig`
- Dropped column `TgF` in the processed files.
...
### Measurement 02 (`pomiar_nr2.csv`)

- Source encoding / format: `cp1250`
- Removed comment header (protocol text only; not stored).
- No `TgF` column in the source.
...
### Measurement 03 (`pomiar_nr3.xlsx`)

- Source encoding / format: `xlsx`
- Excel sheet names: ['chodzenie-bieganie-chodzenie']
- No `TgF` column in the source.
...
### Measurement 05 (`pomiar_nr5.csv`)

- Source encoding / format: `cp1250`
- Removed 2 free-text observer note cell(s) from trailing columns (route/speed notes; incidental place names redacted).
- No `TgF` column in the source.
